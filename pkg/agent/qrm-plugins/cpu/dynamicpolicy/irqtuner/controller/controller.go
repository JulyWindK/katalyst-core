...
func NewNicIrqTuningManager(conf *config.IrqTuningConfig, nic *machine.NicBasicInfo, irqAffSockets []int, order ExclusiveIrqCoresSelectOrder) (*NicIrqTuningManager, error) {
	...
	return &NicIrqTuningManager{
		...
		NicThroughputClassSwitchStat: NicThroughputClassSwitchStat{
...
func (ic *IrqTuningController) classifyNicsByThroughput(oldIndicatorsStats *IndicatorsStats) {
	...
	for _, nic := range ic.Nics {
		...
		if pps <= ic.conf.ThrouputClassSwitchConf.LowThroughputEnterThresholds.RxPPSThresh {
			nic.LowThroughputSuccCount++
			if nic.LowThroughputSuccCount >= ic.conf.ThrouputClassSwitchConf.LowThroughputEnterThresholds.SuccessiveCount {
...
func (ic *IrqTuningController) periodicTuningIrqCoresExclusive() {
	...
	//  tunine irq affinity for low throughput nics
	if err := ic.TuneIrqAffinityForLowThroughputNics(); err != nil {
		klog.Errorf("failed to TuneIrqAffinityForLowThroughputNics, err %v", err)
		ic.emitErrMetric(irqtuner.TuneIrqAffinityForAllNicsWithBalanceFairPolicyFailed, irqtuner.IrqTuningError)
	}
