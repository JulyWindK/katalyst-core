...
func (c *IRQCoreNetOverloadThresholds) ApplyConfiguration(conf *crd.DynamicConfigCRD) {
	if itc := conf.IRQTuningConfiguration; itc != nil &&
		itc.Spec.Config.IRQCoreNetOverLoadThresh != nil {
		config := itc.Spec.Config.IRQCoreNetOverLoadThresh
		if config.IRQCoreSoftNetTimeSqueezeRatio != nil {
			c.IRQCoreSoftNetTimeSqueezeRatio = *config.IRQCoreSoftNetTimeSqueezeRatio
		}
	}
}
