...
func GetIrqAffinityCPUs(irq int) ([]int64, error) {
	...
	smpAffinityListPath, err := procm.GetPorcInterruptAffinityCPUs(irq)
	if err != nil {
		return nil, err
	}
...
func setNicRxQueueRPS(nic *NicBasicInfo, queue int, destCpus []int64) error {
	...
	destCPUSets, err := NewCPUSetInt64(destCpus...)
	if err != nil {
		return fmt.Errorf("failed to new cpu set int64(%v), err %v", destCpus, err)
	}
	if err = sysm.SetNicRxQueueRPS(nsc.sysMountDir, nic.Name, queue, destCPUSets.String()); err != nil {
		return fmt.Errorf("failed to set nic %s rx queue %d rps to %v, sys mount dir: %v, err %v", nic.Name, queue, destCpus, nsc.sysMountDir, err)
	}
...
func SetNicRxQueueRPS(nic *NicBasicInfo, queue int, destCpus []int64) error {
	return setNicRxQueueRPS(nic, queue, destCpus)
}

func ClearNicRxQueueRPS(nic *NicBasicInfo, queue int) error {
	return setNicRxQueueRPS(nic, queue, []int64{})
}
